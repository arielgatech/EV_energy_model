...
training_cycle = listdir('Power_split_HEV_csv')
randomindex = []
composite_cycle = None
...
for cyc in training_cycle:
    ...
    cycle_to_process = read_csv('Power_split_HEV_csv/' + cyc, sep = ',')
    cycle_to_process = cycle_to_process[cycle_to_process['Acceleration(mph/s)'].abs() < 10]
    cycle_to_process = cycle_to_process[cycle_to_process['elec_energy(J)'].abs() < 3000]    
...
plt.subplot(121)
plt.boxplot(composite_cycle['fuel_rate(J)']*10,whis=1.5, sym='')
plt.xlabel('Tukey method')
plt.ylabel('Tractive power')
plt.ylim([0,200000])
plt.subplot(122)
plt.boxplot(composite_cycle['fuel_rate(J)']*10,whis=[0, 0.99], sym='')
...
elec_quartile_1, elec_quartile_3 = composite_cycle['elec_energy(J)'].quantile(0.25), composite_cycle['elec_energy(J)'].quantile(0.75)
elec_iqr = elec_quartile_3 - elec_quartile_1
elec_lower_bound = elec_quartile_1 - (elec_iqr * 1.5)
elec_upper_bound = elec_quartile_3 + (elec_iqr * 1.5)

composite_cycle = composite_cycle[(composite_cycle['tracpower(watt)'] >= lower_bound) & (composite_cycle['tracpower(watt)'] <= upper_bound)]


#composite_cycle = composite_cycle[(composite_cycle['elec_energy(J)'] >= elec_lower_bound) & (composite_cycle['elec_energy(J)'] <= elec_upper_bound)]
...
#composite_cycle = composite_cycle.loc[composite_cycle['SOC']>=0.3]
...
#plt.ylim([0, 0.016])
    plt.savefig('PS_HEV_PLOT/psfinal_elec_' + str(ctrl) + '.jpg', bbox_inches = 'tight', dpi = 200)
    plt.show()
# <codecell>
#training model
...
composite_cycle.loc[:, 'SOC_botton'] = 0 * (composite_cycle.loc[:, 'SOC'] - 0.4 >= 0) + composite_cycle.loc[:, 'SOC'] * (composite_cycle.loc[:, 'SOC'] - 0.4 < 0)
composite_cycle.loc[:, 'SOC_upper'] = 1 * (composite_cycle.loc[:, 'SOC'] - 0.6 >= 0) + 0 * (composite_cycle.loc[:, 'SOC'] - 0.6 < 0)
composite_cycle.loc[:, 'SOC_medium'] = composite_cycle.loc[:, 'SOC'] * ((composite_cycle.loc[:, 'SOC'] - 0.4 >= 0) & (composite_cycle.loc[:, 'SOC'] - 0.6 < 0)) + 0 * ((composite_cycle.loc[:, 'SOC'] - 0.6 >= 0) | (composite_cycle.loc[:, 'SOC'] - 0.4 < 0))
composite_cycle.loc[:, 'SOC_low'] = composite_cycle.loc[:, 'SOC'] * (composite_cycle.loc[:, 'SOC'] - 0.5 < 0) + 0 * (composite_cycle.loc[:, 'SOC'] - 0.5 >= 0)
composite_cycle.loc[:, 'Acc_buffer'] = composite_cycle.loc[:, 'Acceleration(mph/s)'] * (composite_cycle.loc[:, 'Acceleration(mph/s)'] <= 0) * (composite_cycle.loc[:, 'Acceleration(mph/s)'] >= -1) + 0 * (composite_cycle.loc[:, 'SOC'] > 0) + 0 * (composite_cycle.loc[:, 'SOC'] < -1)


kf = KFold(n_splits=10)

X = composite_cycle[['SOC_high', 'SOC_botton', 'SOC', 'SOC_upper', 'SOC_medium', 'SOC_low', 'Speed(mph)', 'Acceleration(mph/s)', 'VSP', 'max_VSP', 'min_VSP', 'Acc_buffer']] # cannot add more
...
eng_control = logit.fit(X_train[['SOC_high','Speed(mph)', 'VSP']], y_train['eng_on'])
print(logit.coef_, logit.intercept_)
y_train.loc[:, 'eng_on_pred'] = logit.predict(X_train[['SOC_high', 'Speed(mph)', 'VSP']])
...
y_test.loc[:, ['eng_off_pred', 'eng_on_pred']] = logit.predict_proba(X_test[['SOC_high', 'Speed(mph)', 'VSP']])
...
elec_pos = lm.fit(X_train.loc[(X_train['VSP']>=0) & (y_train['eng_on']==0), ['VSP']], y_train.loc[(X_train['VSP']>=0) & (y_train['eng_on']==0), 'elec_energy(J)'])
y_train.loc[(X_train['VSP']>=0) & (y_train['eng_on']==0), 'elec_rate_pred'] = lm.predict(X_train.loc[(X_train['VSP']>=0) & (y_train['eng_on']==0), ['VSP']])
...
y_test.loc[(X_test['VSP']>=0), 'elec_rate_eng_off_pred'] = lm.predict(X_test.loc[(X_test['VSP']>=0), ['VSP']])


elec_neg = lm.fit(X_train.loc[(X_train['VSP']<0) & (y_train['eng_on']==0), ['SOC_high', 'SOC_low', 'VSP']], y_train.loc[(X_train['VSP']<0) & (y_train['eng_on']==0), 'elec_energy(J)'])
y_train.loc[(X_train['VSP']<0) & (y_train['eng_on']==0), 'elec_rate_pred'] = lm.predict(X_train.loc[(X_train['VSP']<0) & (y_train['eng_on']==0), ['SOC_high', 'SOC_low', 'VSP']])
print(lm.coef_, lm.intercept_)
...
y_test.loc[(X_test['VSP']<0), 'elec_rate_eng_off_pred'] = lm.predict(X_test.loc[(X_test['VSP']<0), ['SOC_high', 'SOC_low', 'VSP']])
...
mot1_control = logit.fit(X_train.loc[y_train['eng_on'] == 1, ['Speed(mph)', 'VSP']], y_train.loc[y_train['eng_on'] == 1, 'mot1_control'])
print(logit.coef_, logit.intercept_)
y_train.loc[y_train['eng_on'] == 1, 'mot1_control_pred'] = logit.predict(X_train.loc[y_train['eng_on'] == 1, ['Speed(mph)', 'VSP']])
...
y_test.loc[:, ['mot1_neg_pred','mot1_pos_pred']] = logit.predict_proba(X_test.loc[:, ['Speed(mph)', 'VSP']])
...
motoring_fuel = lm.fit(X_train.loc[motoring_loc, ['SOC_high', 'SOC_low', 'VSP']], y_train.loc[motoring_loc, 'fuel_rate(J)'])
print(lm.coef_, lm.intercept_)
y_train.loc[motoring_loc, 'fuel_rate_pred'] = lm.predict(X_train.loc[motoring_loc, ['SOC_high', 'SOC_low', 'VSP']])
...
y_test.loc[:, 'fuel_rate_mot_pos_pred'] = lm.predict(X_test.loc[:, ['SOC_high', 'SOC_low', 'VSP']])
#
#
#3.2.2 electric model under engine on/ motor 1 positive
motoring_low_soc_elec = lm.fit(X_train.loc[motoring_loc, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'Acceleration(mph/s)']], y_train.loc[motoring_loc, 'elec_energy(J)'])
print(lm.coef_, lm.intercept_)
y_train.loc[motoring_loc, 'elec_rate_pred'] = lm.predict(X_train.loc[motoring_loc, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'Acceleration(mph/s)']])
...
y_test.loc[:, 'elec_rate_mot_pos_pred'] = lm.predict(X_test.loc[:, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'Acceleration(mph/s)']])
#
...
#generating_pos_vsp_loc = (X_train['VSP']>=0) & (generating_loc)
###generating_pos_vsp_test_loc = (X_test['VSP']>=0) & (y_test['mot1_control_pred']==0) & (y_test['eng_on_pred']==1)
#generating_neg_vsp_loc = (X_train['VSP']<0) & (generating_loc)
...
generating_fuel = lm.fit(X_train.loc[generating_loc, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'VSP']], y_train.loc[generating_loc, 'fuel_rate(J)'])
print(lm.coef_, lm.intercept_)
y_train.loc[generating_loc, 'fuel_rate_pred'] = lm.predict(X_train.loc[generating_loc, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'VSP']])
...
y_test.loc[:, 'fuel_rate_mot_neg_pred'] = lm.predict(X_test.loc[:, ['SOC_botton', 'SOC_medium', 'SOC_upper', 'VSP']])
...
generating_elec = lm.fit(X_train.loc[generating_loc, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper']], y_train.loc[generating_loc, 'elec_energy(J)'])
print(lm.coef_, lm.intercept_)
y_train.loc[generating_loc, 'elec_rate_pred'] = lm.predict(X_train.loc[generating_loc, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper']])
...
y_test.loc[:, 'elec_rate_mot_neg_pred'] = lm.predict(X_test.loc[:, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper']])
#
