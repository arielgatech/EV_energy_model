...
training_cycle = listdir('Power_split_PHEV_csv')
randomindex = []
composite_cycle = None
...
for cyc in training_cycle:
    ...
    cycle_to_process = read_csv('Power_split_PHEV_csv/' + cyc, sep = ',')
...
#plt.ylim([0, 0.016])
#plt.savefig('PS_PHEV_PLOT/psfinal_elec_VS_SOC_VSP.jpg', bbox_inches = 'tight', dpi = 200)
plt.show()

to_plot = composite_cycle[['SOC', 'Speed(mph)', 'Acceleration(mph/s)','VSP', 'eng_on', 'mot_command', 'mot2_command']].sample(n=10000)

#fig= plt.figure(figsize=(10, 4))
#plt.subplot(121)
#sns.distplot(to_plot.loc[to_plot['eng_on']==0, 'mot_command'])
#sns.distplot(to_plot.loc[to_plot['eng_on']==1, 'mot_command'])
#plt.xlabel('motor output percent power')
#plt.ylabel('relative density')
#plt.legend(['engine off', 'engine on'])
#plt.title("motor 1 control")
#
#plt.subplot(122)
#sns.distplot(to_plot.loc[to_plot['eng_on']==0, 'mot2_command'])
#sns.distplot(to_plot.loc[to_plot['eng_on']==1, 'mot2_command'])
#plt.xlabel('motor output percent power')
#plt.ylabel('relative density')
#plt.legend(['engine off', 'engine on'])
#plt.title("motor 2 control")
#plt.savefig('PS_PHEV_control.jpg', bbox_inches = 'tight', dpi=500)
#plt.show()
#plt.scatter(to_plot['mot_command'], to_plot['mot2_command'], alpha=0.1)
#plt.show()

#sample_cycle = composite_cycle.sample(n=100000)
sample_cycle = sample_cycle[['road_type', 'SOC', 'Speed(mph)','Acceleration(mph/s)', 'road_grade(rad)', 'VSP', 'mot_command', 
                             'eng_on', 'fuel_rate(J)', 'elec_energy(J)']]
...
plt.savefig('PS_PHEV_Corr_matrix.jpg', bbox_inches = 'tight', dpi=500)
plt.show()
# <codecell>
#training model
...
composite_cycle.loc[:, 'SOC_botton'] = 0 * (composite_cycle.loc[:, 'SOC'] - 0.3 >= 0) + composite_cycle.loc[:, 'SOC'] * (composite_cycle.loc[:, 'SOC'] - 0.3 < 0)
composite_cycle.loc[:, 'SOC_upper'] = 1 * (composite_cycle.loc[:, 'SOC'] - 0.36 >= 0) + 0 * (composite_cycle.loc[:, 'SOC'] - 0.36 < 0)
composite_cycle.loc[:, 'SOC_medium'] = composite_cycle.loc[:, 'SOC'] * ((composite_cycle.loc[:, 'SOC'] - 0.3 >= 0) & (composite_cycle.loc[:, 'SOC'] - 0.36 < 0)) + 0 * ((composite_cycle.loc[:, 'SOC'] - 0.36 >= 0) | (composite_cycle.loc[:, 'SOC'] - 0.3 < 0))
...
X = composite_cycle[['SOC_high', 'SOC_botton', 'SOC', 'SOC_upper', 'SOC_medium', 'SOC_lower', 'Speed(mph)', 'Acceleration(mph/s)', 'VSP', 'max_VSP', 'min_VSP']] # cannot add more
...
generating_elec = lm.fit(X_train.loc[generating_loc, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper','VSP']], y_train.loc[generating_loc, 'elec_energy(J)'])
print(lm.coef_, lm.intercept_)
y_train.loc[generating_loc, 'elec_rate_pred'] = lm.predict(X_train.loc[generating_loc, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper','VSP']])
...
y_test.loc[:, 'elec_rate_mot_neg_pred'] = lm.predict(X_test.loc[:, ['Speed(mph)', 'SOC_botton', 'SOC_medium', 'SOC_upper', 'VSP']])
#
